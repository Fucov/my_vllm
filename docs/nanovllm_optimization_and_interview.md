...
本文基于当前仓库真实代码、Git 历史和可见文件，对 nano-vLLM 推理优化项目做一次面向“大模型推理框架开发岗位”的代码审计。审计范围覆盖从 baseline commit `ffa7349` 到当前 commit `49d54af` 的全部有效改动，并补充分析 `logs/full_bench_20260619_235153/` 下的完整 benchmark 结果。
...
| 工程性补丁 | 2 项：matrix benchmark 子进程隔离；完整 benchmark 脚本按 workload/variant 独立落盘日志 |
| 当前仓库可验证性能数据 | `logs/full_bench_20260619_235153/` 包含 12 个配置的 JSONL/log/cmd/meta，每个配置 repeat=5，可复核本轮实验 |
| 可写入简历的稳妥说法 | “实现原型并建立 benchmark 矩阵；在 RTX PRO 4000 Blackwell SFF、Qwen3-0.6B、seq=128、repeat=5 的 shared-prefix workload 下，late-merge 观察到 peak used KV blocks -25.12%、吞吐 +5.71%、decode P95 -7.46%” |
...
3. `Safe CUDA Graph` 增加 runtime eligibility 检查和 fallback 统计。本轮 12 组实验 graph fallback 均为 0，只能说明这些 workload 未触发 fallback，不能替代失败注入实验。
...
| feature 分支 | `9f06488`、`ea73880`、`e7a3d91`、`07962aa`、`49d54af` 是本轮优化相关提交 |
...
docs/nanovllm_optimization_and_interview.md        | 1555 ++++++++++++++++++++
...
scripts/run_full_bench_matrix.sh                   |  291 ++++
tests/test_bench_cli.py                            |   49 +
14 files changed, 3544 insertions(+), 42 deletions(-)
```
...
| A | `docs/nanovllm_optimization_and_interview.md` |
...
| A | `scripts/run_full_bench_matrix.sh` |
...
| `49d54af` | 文档与实验脚本 | 新增审计文档和 `scripts/run_full_bench_matrix.sh`，按 workload/variant 独立保存 cmd/meta/log/jsonl |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Benchmark & Metrics | `bench.py`、`llm_engine.py`、`scheduler.py`、`block_manager.py`、`model_runner.py` | `run_once()`、`LLMEngine.metrics()`、各 `reset_metrics()` | benchmark 增强、统计增强 | 原始仓库缺少可复现实验尺子 | 量化吞吐、TTFT、prefill/decode latency、KV block、graph fallback | 已由 `logs/full_bench_20260619_235153/` 产出 12 组 repeat=5 JSONL |
| 2 | Token-budget Fair Chunked Prefill | `config.py`、`scheduler.py`、`bench.py` | `Scheduler._schedule_fair_prefill()` | 调度策略变化 | 长 prompt prefill 可能独占 token budget，短请求等待 | 降低单次 prefill step 长尾，改善混合长度 workload 的调度形态 | mixed-lengths 下 fair prefill mean -12.95%，optimized -12.57%，但吞吐轻微下降 |
| 3 | Shared-prefix Full-block KV Late Merge | `config.py`、`block_manager.py`、`scheduler.py`、`bench.py` | `BlockManager.hash_blocks()`、`_late_merge_block()` | KV Cache 管理变化、数据结构变化 | 多请求共享 prefix 但未在 allocate 阶段命中时，会产生重复 physical KV blocks | 降低 resident used KV blocks，提升 shared-prefix 场景容量和 decode 局部性 | shared-prefix 下 late_merge peak KV blocks -25.12%、吞吐 +5.71%、decode P95 -7.46% |
| 4 | Safe CUDA Graph Eligibility / Fallback | `config.py`、`model_runner.py`、`llm_engine.py`、`bench.py` | `ModelRunner.run_model()`、`capture_cudagraph()` | correctness guardrail、CUDA Graph 相关变化 | decode graph replay 对 batch size、context length、block table shape 敏感 | 避免 ineligible shape 直接 replay，统计 fallback reason | 12 组 benchmark graph fallback 均为 0；未覆盖失败注入 |
| 5 | Matrix Benchmark Subprocess Isolation 与完整运行脚本 | `bench.py`、`tests/test_bench_cli.py`、`scripts/run_full_bench_matrix.sh` | `run_repeated_isolated()`、`build_child_command()`、benchmark shell | 生命周期管理、benchmark 稳定性、日志归档 | 同一进程 repeat 创建/销毁 LLM 后 CUDA/NCCL 状态未完全冷启动；手工命令难追溯 | 提高矩阵实验可运行性和证据可追溯性 | 本轮日志按 workload/variant 独立保存 cmd/meta/log/jsonl |
...
4. 当前已有 GPU benchmark logs/jsonl，但仍缺少自动化 correctness 校验和 Nsight profiling。
...
本轮 `logs/full_bench_20260619_235153` 中，shared-prefix late_merge 与 optimized 的 peak used KV blocks 均相对 baseline 下降 25.12%，与该机制吻合。但这仍是特定模型、硬件、block size 与 workload 下的结果，不能泛化为所有请求。
...
当前仓库已经包含完整 benchmark 输出：

```text
logs/full_bench_20260619_235153/
```

该目录由 `scripts/run_full_bench_matrix.sh` 生成，包含：

1. `env.log`：实验环境、Git commit、Python/PyTorch/CUDA、GPU 与 benchmark 默认参数；
2. 每个 workload/variant 一组 `.cmd.sh`：实际执行命令；
3. 每个 workload/variant 一组 `.meta.txt`：关键配置；
4. 每个 workload/variant 一组 `.log`：`tee` 保存的完整 stdout/stderr；
5. 每个 workload/variant 一组 `.jsonl`：repeat=5 的原始 JSON 记录。

本轮实验环境：

| 项 | 值 |
| --- | --- |
| 运行时间 | `20260619_235153` |
| 运行目录 | `/home/xiaoyi-dev/yangkw/my_vllm` |
| 实验 commit | `49d54af0b8e24a8b03e9d8b284dc54459473ee2c` |
| 分支 | `feature` |
| 模型 | `/home/xiaoyi-dev/models/Qwen3-0.6B/` |
| Python | 3.11.15 |
| PyTorch | 2.7.0+cu128 |
| CUDA runtime | 12.8 |
| Driver / CUDA | NVIDIA-SMI 595.71.05 / CUDA 13.2 |
| GPU | NVIDIA RTX PRO 4000 Blackwell SFF Edition，显存 24467 MiB |
| num_seqs | 128 |
| repeat | 5 |
| max_model_len | 4096 |
...
```text
12 个 JSONL 文件 = 3 个 workload × 4 个 variant
每个 JSONL 文件 5 行 = repeat=5
graph_fallbacks 在所有 12 组中均值均为 0
```

### 10.5 实验结果总表

下表由 `logs/full_bench_20260619_235153/*.jsonl` 聚合得到。所有数值为 5 次 repeat 的均值；`ΔThr` 和 `ΔKV` 均相对同一 workload 的 baseline 计算。

| Workload | Config | Runs | Throughput tok/s | ΔThr | TTFT mean/P95 s | Prefill mean/P95 s | Decode mean/P95 s | Peak KV blocks | ΔKV | Prefix hit | Late merge reclaimed | Graph fallback |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| random | baseline | 5 | 2451.64 | +0.00% | 1.0031/1.6420 | 0.3281/0.3807 | 0.0202/0.0283 | 361.4 | +0.00% | 0.0 | 0.0 | 0.0 |
| random | fair | 5 | 2421.74 | -1.22% | 1.0351/1.6700 | 0.3337/0.3937 | 0.0204/0.0286 | 361.4 | +0.00% | 0.0 | 0.0 | 0.0 |
| random | late_merge | 5 | 2419.99 | -1.29% | 1.0242/1.6747 | 0.3346/0.3892 | 0.0204/0.0285 | 361.4 | +0.00% | 0.0 | 0.0 | 0.0 |
| random | optimized | 5 | 2415.25 | -1.48% | 1.0378/1.6750 | 0.3347/0.3951 | 0.0205/0.0286 | 361.4 | +0.00% | 0.0 | 0.0 | 0.0 |
| mixed_lengths | baseline | 5 | 2218.78 | +0.00% | 1.1060/1.8521 | 0.3582/0.3914 | 0.0215/0.0306 | 389.2 | +0.00% | 0.0 | 0.0 | 0.0 |
| mixed_lengths | fair | 5 | 2211.23 | -0.34% | 1.1922/1.8418 | 0.3119/0.3959 | 0.0215/0.0305 | 389.2 | +0.00% | 0.0 | 0.0 | 0.0 |
| mixed_lengths | late_merge | 5 | 2206.36 | -0.56% | 1.1143/1.8650 | 0.3607/0.3945 | 0.0216/0.0307 | 389.2 | +0.00% | 0.0 | 0.0 | 0.0 |
| mixed_lengths | optimized | 5 | 2204.00 | -0.67% | 1.1977/1.8496 | 0.3132/0.3982 | 0.0215/0.0305 | 389.2 | +0.00% | 0.0 | 0.0 | 0.0 |
| shared_prefix | baseline | 5 | 3178.20 | +0.00% | 0.7082/0.7589 | 0.3763/0.3926 | 0.0198/0.0277 | 200.6 | +0.00% | 330.6 | 0.0 | 0.0 |
| shared_prefix | fair | 5 | 3170.43 | -0.24% | 0.7226/0.7742 | 0.3841/0.4015 | 0.0198/0.0276 | 203.6 | +1.50% | 327.6 | 0.0 | 0.0 |
| shared_prefix | late_merge | 5 | 3359.72 | +5.71% | 0.7062/0.7566 | 0.3753/0.3924 | 0.0184/0.0256 | 150.2 | -25.12% | 330.6 | 50.4 | 0.0 |
| shared_prefix | optimized | 5 | 3341.24 | +5.13% | 0.7231/0.7747 | 0.3844/0.4022 | 0.0184/0.0256 | 150.2 | -25.12% | 327.6 | 53.4 | 0.0 |

关键 delta：

| Workload | Config | Throughput | TTFT P95 | Prefill Mean | Decode P95 | Peak KV Blocks | 结论 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| random | optimized | -1.48% | +2.01% | +2.01% | +0.96% | +0.00% | 无 prefix 共享时优化组合主要体现为轻微 overhead |
| mixed_lengths | fair | -0.34% | -0.56% | -12.95% | -0.31% | +0.00% | fair chunking 明显降低 prefill step mean，但未转化为吞吐提升 |
| mixed_lengths | optimized | -0.67% | -0.14% | -12.57% | -0.07% | +0.00% | 组合优化保留 prefill mean 收益，但吞吐略降 |
| shared_prefix | late_merge | +5.71% | -0.30% | -0.27% | -7.46% | -25.12% | late merge 是本轮最强证据：KV 水位、吞吐、decode P95 同向改善 |
| shared_prefix | optimized | +5.13% | +2.08% | +2.14% | -7.56% | -25.12% | late merge 收益仍在，但 fair chunking 拉高 TTFT/prefill mean |

### 10.6 统计稳定性

5 次 repeat 的均值与标准差如下：

| Workload | Config | Thr mean±std | TTFT P95 mean±std | Prefill mean±std | Decode P95 mean±std | KV mean±std |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| random | baseline | 2451.64±91.79 | 1.6420±0.0786 | 0.3281±0.0157 | 0.0283±0.0009 | 361.4±14.2 |
| random | fair | 2421.74±90.08 | 1.6700±0.0740 | 0.3337±0.0148 | 0.0286±0.0009 | 361.4±14.2 |
| random | late_merge | 2419.99±89.70 | 1.6747±0.0772 | 0.3346±0.0154 | 0.0285±0.0009 | 361.4±14.2 |
| random | optimized | 2415.25±88.62 | 1.6750±0.0762 | 0.3347±0.0152 | 0.0286±0.0009 | 361.4±14.2 |
| mixed_lengths | baseline | 2218.78±38.41 | 1.8521±0.0335 | 0.3582±0.0226 | 0.0306±0.0005 | 389.2±9.1 |
| mixed_lengths | fair | 2211.23±38.08 | 1.8418±0.0326 | 0.3119±0.0055 | 0.0305±0.0005 | 389.2±9.1 |
| mixed_lengths | late_merge | 2206.36±37.40 | 1.8650±0.0362 | 0.3607±0.0214 | 0.0307±0.0005 | 389.2±9.1 |
| mixed_lengths | optimized | 2204.00±33.91 | 1.8496±0.0361 | 0.3132±0.0058 | 0.0305±0.0005 | 389.2±9.1 |
| shared_prefix | baseline | 3178.20±55.12 | 0.7589±0.0284 | 0.3763±0.0143 | 0.0277±0.0004 | 200.6±5.9 |
| shared_prefix | fair | 3170.43±55.64 | 0.7742±0.0304 | 0.3841±0.0152 | 0.0276±0.0003 | 203.6±5.9 |
| shared_prefix | late_merge | 3359.72±55.09 | 0.7566±0.0306 | 0.3753±0.0152 | 0.0256±0.0003 | 150.2±5.4 |
| shared_prefix | optimized | 3341.24±59.72 | 0.7747±0.0316 | 0.3844±0.0158 | 0.0256±0.0004 | 150.2±5.4 |

解读：

1. `shared_prefix/late_merge` 的吞吐提升约 +5.71%，大于吞吐 std 相对尺度，且 KV blocks -25.12%、decode P95 -7.46% 与机制一致，是本轮最可信收益。
2. `mixed_lengths/fair` 的 prefill mean -12.95%，但 prefill P95 略升、吞吐 -0.34%，说明它改变了 prefill step 形态，却不能在当前 workload 下宣称整体吞吐提升。
3. `random/optimized` 吞吐 -1.48%、TTFT P95 +2.01%，支持“无共享 prefix 场景会有轻微 overhead”的风险判断。
4. 所有配置 graph fallback 为 0，说明当前 shape 均落在 captured/eligible 范围内，但没有验证 fallback 分支的实际行为。
...
```bash
bash scripts/run_full_bench_matrix.sh ~/models/Qwen3-0.6B/
```

如果只想重跑单个配置，可直接执行对应 `.cmd.sh`，例如：

```bash
bash logs/full_bench_20260619_235153/shared_prefix__late_merge__seq128__rep5__tok16384__blk256__chunk1024.cmd.sh
```
...
第一类是轻量单元测试：`tests/test_bench_cli.py`。
...
第二类是 GPU benchmark 原始数据：`logs/full_bench_20260619_235153/*.jsonl`。该目录包含 12 个 workload/variant 组合，每个组合 repeat=5，并保留 `.cmd.sh`、`.meta.txt`、`.log` 和 `env.log`。因此，本轮性能数字可以从当前仓库复核。

### 11.2 性能说法可信度分级

| 说法 | 代码机制是否支持 | 当前仓库实验是否可复核 | 文档建议 |
| --- | --- | --- | --- |
| shared-prefix 下 used KV blocks 降低约 25.12% | 支持，late merge 会释放 duplicate full block | 是，late_merge 与 optimized 均为 -25.12% | 可写入简历，但限定 workload、硬件、模型和 repeat |
| shared-prefix 下吞吐提升约 5.71% | 机制上可能，但吞吐受多因素影响 | 是，late_merge +5.71%，optimized +5.13% | 可写“特定 shared-prefix workload 观察到吞吐提升” |
| shared-prefix 下 decode P95 降低约 7.46% | 可能与 KV footprint 降低相关，但缺少 profiling 因果证明 | 是，late_merge -7.46%，optimized -7.56% | 可写结果，不要写成已证明的 kernel 因果 |
| mixed-lengths 下 prefill mean 降低约 12.5% | fair chunking 机制支持降低单 step prefill | 是，fair -12.95%，optimized -12.57% | 可写“prefill step mean 降低”，但同时说明吞吐未提升 |
| random workload 吞吐轻微退化 | 机制上 random 无 prefix 收益，轻微 overhead 合理 | 是，optimized -1.48% | 作为适用边界写入，不宜掩盖 |
| CUDA Graph fallback 为 0 | 代码可统计 | 是，12 组均为 0 | 可写“本轮 workload 未触发 fallback”，不能证明 fallback 分支正确 |
...
1. “decode P95 改善来自 KV footprint 降低”。这个因果方向合理，但仍需 Nsight 或更细粒度 profiling 支撑。
2. “Fair Chunked Prefill 改善延迟”。本轮 mixed-lengths 中 prefill mean 降低，但 TTFT mean 上升、吞吐略降，应写得克制。
3. “CUDA Graph fallback 安全”。本轮 fallback 为 0，尚未验证 fallback 分支。

不建议写：

1. “显著提升 vLLM 推理性能”，因为本项目是 nano-VLLM 原型，不是正式 vLLM。
2. “证明 decode P95 降低来自 KV block 减少”，缺少 profiling 因果链。
3. “Fair Chunked Prefill 提升整体吞吐”，本轮 mixed-lengths 和 random 都没有支持这个说法。
4. “生产级 Prefix Caching”，当前没有 eviction、cache salt、多 GPU、一致性和并发保护。
...
**推荐回答：** 需要至少三类 workload：random 验证低共享场景，mixed-length 验证调度压力，shared-prefix 验证 prefix cache 场景；每类用 baseline/fair/late_merge/optimized 矩阵和 repeat，固定 seed 起点，保存 JSONL 原始数据。本轮已经保存了 12 组 repeat=5 的 JSONL/log，能说明 shared-prefix 收益不是只看单次运行；但还缺真实线上 trace 和更多并发形态，因此不能说已经证明对所有 workload 都有效。
...
**推荐回答：** 当前每个配置 repeat=5、每轮 128 个请求，已经能初步看均值和标准差，但 P95/P99 的统计稳定性仍有限。正式报告应继续增加 repeat、请求数和真实 trace，并展示 confidence interval，避免单次尾延迟被噪声影响。

**回答依据：** `summarize_results()` L231-L265 会输出 mean/std/min/max；本轮 `logs/full_bench_20260619_235153/*.jsonl` 每个配置有 5 条记录。

**容易踩坑的回答：** “P95 已经完全稳定”。5 次 repeat 只是初步证据。
...
**推荐回答：** 我会先补正确性验证：输出一致性测试、late merge ref_count 单测、hash collision 构造测试、CUDA Graph fallback failure injection。性能侧继续补真实 trace、更多并发规模和 Nsight profiling。然后做 adaptive chunk size 和 decode-first policy，最后再考虑把 late merge 前移为 allocation-time prefix cache 增强，或引入 radix tree。

**回答依据：** 当前仓库已有原始 benchmark logs，最大短板转为 correctness tests、failure injection 和 profiling 证据。
...
实验上，我用 `scripts/run_full_bench_matrix.sh` 在 Qwen3-0.6B、RTX PRO 4000 Blackwell SFF、seq=128、repeat=5 下跑了 3 类 workload × 4 个 variant。最强结果来自 shared-prefix：late_merge 相比 baseline peak used KV blocks 下降 25.12%，吞吐提升 5.71%，decode P95 降低 7.46%。mixed-lengths 下 fair prefill 的 prefill step mean 降低 12.95%，但吞吐略降，所以我把它表述为调度形态改善而不是整体性能提升。

局限是当前原型还不是 production 级：没有多 GPU、eviction、LoRA/cache salt、abort、完整 correctness tests；同时缺少 Nsight profiling，因此不能把 decode P95 改善直接写成已证明的 kernel 级因果。
...
benchmark 设计上，random 检查低共享场景 overhead，mixed-length 检查调度，shared-prefix 检查 KV dedup。当前代码支持 repeat 和 JSONL，并用子进程隔离 matrix，避免 CUDA/NCCL 状态污染。本轮 `logs/full_bench_20260619_235153/` 下保存了每条命令、meta、tee 日志和原始 JSONL，可复核所有表格数字。结果上，shared-prefix late_merge 是最强证据：peak used KV blocks -25.12%，吞吐 +5.71%，decode P95 -7.46%；random optimized 吞吐 -1.48%，说明无共享 prefix 场景存在轻微 overhead；mixed-lengths fair prefill mean -12.95%，但吞吐 -0.34%，所以不能夸大成整体吞吐提升。
...
| --- | --- | --- | --- | --- |
| 实现 shared-prefix full-block KV late-merge | 能 | 能，见 `logs/full_bench_20260619_235153` | 准确，但需标注原型 | 设计并实现 shared-prefix full-block KV late-merge 原型 |
...
| 降低 used KV blocks | 机制能支持 | 能，shared-prefix late_merge/optimized 均为 -25.12% | 需限定 workload | 在 shared-prefix workload 下观察到 peak used KV blocks 下降 25.12% |
| 吞吐提升 | 代码不能直接证明 | 能，shared-prefix late_merge +5.71%、optimized +5.13% | 不能泛化到 random/mixed | 在 shared-prefix workload 下观察到吞吐提升 5.71% |
| decode P95 降低 | 代码不能直接证明因果 | 能，shared-prefix late_merge -7.46%、optimized -7.56% | 因果仍需 profiling | 在 shared-prefix workload 下 decode P95 下降 7.46%，原因需结合 profiling 进一步验证 |
| 实现 token-budget chunked prefill scheduler | 能 | 能，mixed-lengths fair prefill mean -12.95%，但吞吐 -0.34% | 不能写吞吐提升 | 实现 token-budget fair chunked prefill 原型，在 mixed-lengths 下降低 prefill step mean 12.95% |
...
\begin{itemize}[leftmargin=1.5em, itemsep=0.15ex, parsep=0.15ex, topsep=0.2ex]
    \item \textbf{Benchmark 与指标体系：} 重构 \texttt{bench.py} 并新增完整运行脚本，支持 random / mixed-length / shared-prefix 三类 workload 及 baseline / fair / late-merge / optimized 矩阵对比，输出吞吐、TTFT、prefill/decode step latency、KV block 水位、prefix hit 与 CUDA Graph fallback 等指标；通过子进程隔离 repeat 实验，并按配置独立保存 command、meta、tee log 与 JSONL 原始数据。
    \item \textbf{Token-budget Chunked Prefill：} 在 Scheduler 中实现 \texttt{prefill\_policy=fair} 原型，基于 \texttt{max\_num\_batched\_tokens} 与 \texttt{prefill\_chunk\_size} 将长 prompt prefill 拆分为多轮 chunk；在 Qwen3-0.6B、seq=128、repeat=5 的 mixed-length workload 下，prefill step mean latency 下降 12.95%，但整体吞吐略降 0.34%，因此将其定位为调度形态优化而非吞吐优化。
    \item \textbf{Shared-prefix KV Late Merge：} 在 BlockManager 中实现 full-block 级 KV 去重原型，使用父 block hash + 当前 block token 构造链式 hash，hash 命中后进行 token 校验，并通过 block table 重定向与 ref-count 维护回收重复 physical KV blocks；在 shared-prefix workload 下观察到 peak used KV blocks 下降 25.12%、吞吐提升 5.71%、decode P95 latency 下降 7.46%。
...
```text
nano-VLLM 推理优化原型：实现 benchmark 矩阵、token-budget chunked prefill、shared-prefix full-block KV late merge 与 CUDA Graph eligibility/fallback；在 shared-prefix workload 下观察到 KV block 水位 -25.12%、吞吐 +5.71%，当前为单机单模型原型。
```

## 17. 当前仍未解决的问题

1. 已有原始 benchmark logs/jsonl，但缺少自动化汇总脚本和可视化报告，当前表格仍由手工聚合生成。
2. 缺少输出一致性测试，不能证明 late merge 与 graph fallback 对生成结果无影响。
3. 缺少 ref_count、block table redirect、hash collision、preemption 的单元测试。
4. 缺少 CUDA Graph fallback failure injection，本轮 12 组实验 graph fallback 均为 0。
...
1. 补分析工具：新增 JSONL 自动汇总脚本，直接生成 Markdown/CSV 表格、delta、std 和 confidence interval。
...
| 完整 benchmark 脚本 | `scripts/run_full_bench_matrix.sh` | shell script | L1-L291 | `WORKLOADS`、`VARIANTS` | 生成 env/cmd/meta/log/jsonl |
| 环境记录 | `scripts/run_full_bench_matrix.sh` | env block | L60-L117 | `env.log` | 记录 Git、Python、Torch、CUDA、nvidia-smi |
| 单配置运行 | `scripts/run_full_bench_matrix.sh` | `run_case` | L170-L257 | `.cmd.sh`、`.meta.txt`、`tee` | 每个 workload/variant 独立落盘 |
...
| `49d54af` | `scripts/run_full_bench_matrix.sh` | 完整 benchmark 运行脚本，按配置独立保存命令、meta、日志和 JSONL |
| `49d54af` | `docs/nanovllm_optimization_and_interview.md` | 审计与面试准备文档 |
...
for f in logs/full_bench_20260619_235153/*.jsonl; do printf '%s ' "$f"; wc -l < "$f"; done
nl -ba bench.py
...
nl -ba scripts/run_full_bench_matrix.sh
```
...
| 是否覆盖全部 commits | 是，覆盖 `9f06488..49d54af` |
| 是否区分功能修改与统计修改 | 是 |
| 是否有未提供代码证据的结论 | 性能数字均来自 `logs/full_bench_20260619_235153/*.jsonl`；因果解释仍标注 profiling 边界 |
| 是否虚构指标 | 否，表格数字来自 repeat=5 JSONL 聚合 |
